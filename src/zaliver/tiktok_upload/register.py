"""Регистрация аккаунта TikTok (после входа в Gmail)."""
...
_IG_CAPTCHA_FALLBACK_URLS = (
    "https://www.tiktok.com/accounts/emailsignup/",
...
# Ручная капча ждётся внутри воркера (слот занят), пока wait_tiktok_manual_captcha.
KEEP_PROFILE_OPEN_AFTER_TT_REGISTER = False


class TikTokSmsCaptchaError(RuntimeError):
    """Циферная image-капча (/accounts/suspended) — авторег не продолжаем."""

    def __init__(self, detail: str = "") -> None:
        base = TT_REGISTER_SMS_ERROR_TAG
    ...
    def matches(cls, err: str) -> bool:
        text = err or ""
        return TT_REGISTER_SMS_ERROR_TAG in text


class TikTokRegistrationFailedError(RuntimeError):
    """
    ...
    def matches(cls, err: str) -> bool:
        text = err or ""
        return TT_REGISTER_ERROR_TAG in text and TT_REGISTER_SMS_ERROR_TAG not in text


class TikTokAlreadyLoggedInError(RuntimeError):
    """В профиле уже выполнен вход в TikTok — регистрация не нужна."""

    def __init__(self, username: str = "", detail: str = "") -> None:
        self.username = (username or "").strip().lstrip("@")
        bits = ["TikTok: уже выполнен вход в аккаунт"]
        if self.username:
...
def abort_if_tiktok_sms_image_captcha(page) -> None:
    """
    ...
    try:
        ...
        url = ""
    _log(
        f"TikTok: human/SMS captcha — авторег остановлен "
        f"({TT_REGISTER_SMS_ERROR_TAG}), URL={url!r}"
    )
    raise TikTokSmsCaptchaError(f"URL={url!r}")
...
def abort_if_tiktok_registration_failed(page) -> None:
    ...
    try:
        ...
        url = ""
    _log(
        f"TikTok: баннер ошибки регистрации — стоп "
        f"({TT_REGISTER_ERROR_TAG}), URL={url!r}"
    )
    raise TikTokRegistrationFailedError(f"URL={url!r}")
...
_LOG_IN_RE = re.compile(
    r"^войти$|^войти в аккаунт$|^вход$|^log\s*in$|^sign\s*in$",
    re.IGNORECASE,
)
...
_COOKIE_CONSENT_HEADING_RE = re.compile(
    ...
    r"разрешить\s+использование\s+файлов\s+cookie\s+от\s+tiktok|"
    r"использовать\s+файлы\s+cookie|"
    r"cookie[s]?\s+(by|from)\s+tiktok|"
    r"cookies?\s+from\s+tiktok\s+on\s+this\s+browser|"
    r"файлов\s+cookie\s+от\s+tiktok\s+в\s+этом\s+браузере",
    re.IGNORECASE,
)
...
def _fill_labeled_input(page, label_patterns: list[str], value: str) -> None:
    ...
    for label in label_patterns:
        ...
        try:
            ...
            pass
    raise RuntimeError(f"TikTok: не найдено поле ввода ({label_patterns!r})")
...
def _select_combobox_option(page, aria_patterns: list[str], option_texts: list[str]) -> None:
    ...
    for aria in aria_patterns:
        ...
        try:
            ...
            continue
    if combo is None:
        raise RuntimeError(f"TikTok: не найден combobox ({aria_patterns!r})")
    ...
    for text in option_texts:
        ...
        try:
            ...
            pass
    raise RuntimeError(
        f"TikTok: не выбрана опция {option_texts!r} в combobox {aria_patterns!r}"
    )


def _fill_birthday(page) -> tuple[int, int, int]:
    day, month, year = random_birthday()
    _log(f"TikTok: дата рождения {day:02d}.{month:02d}.{year}")
...
def _is_accounts_suspended(page) -> bool:
    """URL /accounts/suspended — human-check / циферная капча TikTok."""
    url = ""
    try:
...
def dismiss_tiktok_scraping_warning_if_present(page) -> bool:
    """
    ...
    if not _scraping_warning_visible(page):
        return False
    _log(
        "TikTok: предупреждение об автоматизации "
    ...
    try:
        ...
        if btn.count() > 0 and btn.is_visible(timeout=800):
            ...
            _log("TikTok: scraping_warning — нажали «Закрыть».")
    ...
    try:
        ...
        if btn.count() > 0 and btn.is_visible(timeout=500):
            ...
            _log("TikTok: scraping_warning — нажали «Закрыть» (button).")
            page.wait_for_timeout(1000)
            return True
    except Exception:
        pass
    if _click_by_text(page, _CLOSE_BTN_RE, prefer_link=False):
        _log("TikTok: scraping_warning — нажали «Закрыть» (текст).")
        page.wait_for_timeout(1000)
        return True
    _log("TikTok: scraping_warning виден, но «Закрыть» не нажалась.")
    return False
...
def _signup_form_visible(page) -> bool:
    ...
    try:
        if page.get_by_text(
            re.compile(r"зарегистрируйтесь|sign\s*up\s+for\s+tiktok", re.I)
...
() => {
  const reserved = new Set([
    ...
    'friends', 'foryou', 'fyp', 'tiktokstudio', 'messages', 'upload',
  ]);
  const pick = (href) => {
    if (!href) return '';
    try {
  ...
  // Левый сайдбар: a[data-e2e="nav-profile"] href="/@username".
  const side = document.querySelector('[data-key-interaction="side_nav"]');
  const scoped = side || document;
  const navProfileSels = [
    'a[data-e2e="nav-profile"]',
    '[data-e2e="nav-profile"] a[href^="/@"]',
    'a[href^="/@"]',
  ];
  for (const sel of navProfileSels) {
    for (const a of Array.from(scoped.querySelectorAll(sel))) {
      const name = pick(a.getAttribute('href') || a.href || '');
      if (name) return name;
    }
  }

  // aria-label / подпись Profile | Профиль → ближайший a[href].
  const isProfileLabel = (s) => /^(profile|профиль)$/i.test((s || '').trim());
  for (const el of Array.from(scoped.querySelectorAll('a, button'))) {
    if (!isProfileLabel(el.getAttribute('aria-label'))) continue;
    ...
    if (wrap) {
      const n3 = pick(wrap.getAttribute('href') || wrap.href || '');
      if (n3) return n3;
    }
  }
  return '';
...
_OPEN_IG_APP_RE = re.compile(
    r"^открыть\s+tiktok$|^open\s+(the\s+)?tiktok(\s+app)?$",
    re.IGNORECASE,
)


def _tiktok_login_form_visible(page) -> bool:
...
def _is_mobile_logged_out_landing(page) -> bool:
    ...
    «Войти» + «зарегистрироваться» (+ часто «Открыть TikTok»).
    """
...
def _click_mobile_landing_log_in(page) -> bool:
    """На splash нажать «Войти» (не «Открыть TikTok»)."""
    try:
...
def _tiktok_session_cookie_present(page) -> bool:
...
_TIKTOK_SIDEBAR_LOGIN_JS = """
() => {
  const isLoginLabel = (s) => /^(log\\s*in|sign\\s*in|войти|войти в аккаунт|вход)$/i.test((s || '').trim());
  const side = document.querySelector('[data-key-interaction="side_nav"]');
  if (!side) return false;
  if (side.querySelector(
    '[data-e2e="top-login-button"], [data-e2e="nav-login"], a[href*="/login"]'
  )) return true;
  const nodes = side.querySelectorAll('a, button');
  for (const el of nodes) {
    if (isLoginLabel(el.getAttribute('aria-label'))) return true;
    const label = el.querySelector('.TUXButton-label');
    if (label && isLoginLabel(label.textContent)) return true;
  }
  return false;
}
"""

_TIKTOK_LOGGED_IN_NAV_JS = """
() => {
  const isLoginLabel = (s) => /^(log\\s*in|sign\\s*in|войти|войти в аккаунт|вход)$/i.test((s || '').trim());
  const side = document.querySelector('[data-key-interaction="side_nav"]');
  if (!side) return false;
  ...
  const nodes = side.querySelectorAll('a, button');
  for (const el of nodes) {
    if (isLoginLabel(el.getAttribute('aria-label'))) return false;
    const label = el.querySelector('.TUXButton-label');
    if (label && isLoginLabel(label.textContent)) return false;
  }
  return !!side.querySelector('[data-e2e="nav-foryou"], [data-e2e="nav-upload"]');
}
"""


def _tiktok_sidebar_login_visible(page) -> bool:
    """Левый сайдбар с кнопкой Log in / Войти (сессии нет)."""
    try:
        if page.evaluate(_TIKTOK_SIDEBAR_LOGIN_JS):
            return True
    except Exception:
        pass
    strong = (
        '[data-key-interaction="side_nav"] [data-e2e="top-login-button"]',
        '[data-key-interaction="side_nav"] [data-e2e="nav-login"]',
        '[data-key-interaction="side_nav"] button[aria-label="Log in" i]',
        '[data-key-interaction="side_nav"] button[aria-label="Sign in" i]',
        '[data-key-interaction="side_nav"] button[aria-label="Войти" i]',
        '[data-key-interaction="side_nav"] button[aria-label="Вход" i]',
        '[data-key-interaction="side_nav"] a[aria-label="Log in" i]',
        '[data-key-interaction="side_nav"] a[aria-label="Sign in" i]',
        '[data-key-interaction="side_nav"] a[aria-label="Войти" i]',
        '[data-key-interaction="side_nav"] a[aria-label="Вход" i]',
    )
    for sel in strong:
        try:
            ...
            continue
    try:
        side = page.locator('[data-key-interaction="side_nav"]')
        if int(side.count()) <= 0:
            return False
        login = side.get_by_text(re.compile(r"^(Log in|Sign in|Войти|Вход)$", re.I))
        return int(login.count()) > 0
    except Exception:
        return False


def _tiktok_logged_in_nav_visible(page) -> bool:
    """Залогиненный сайдбар: нет кнопки Log in / Войти.

    На фоновых вкладках Chromium часто не считает элементы visible —
    достаточно наличия в DOM.
    """
    if _tiktok_sidebar_login_visible(page):
        return False
    try:
        if page.evaluate(_TIKTOK_LOGGED_IN_NAV_JS):
            return True
    except Exception:
        pass
    try:
        ...
        if int(side.count()) <= 0:
            return False
        foryou = side.locator(
            '[data-e2e="nav-foryou"], [data-e2e="nav-upload"]'
        )
        return int(foryou.count()) > 0
    except Exception:
        return False


def _tiktok_already_logged_in(page) -> bool:
    """
    True, если в профиле уже открыта лента / UI залогиненного TikTok
    ...
    if _signup_form_visible(page):
        return False
    if _tiktok_login_form_visible(page):
        return False
    if _tiktok_sidebar_login_visible(page):
        return False
    url = ""
    ...
    if _tiktok_session_cookie_present(page):
        return True
    return _tiktok_logged_in_nav_visible(page)


def _raise_if_already_logged_in(page) -> None:
    if not _tiktok_already_logged_in(page):
        return
    ...
    try:
        ...
        url = ""
    _log(
        "TikTok: обнаружен уже выполненный вход"
        + (f" (@{username})" if username else " (ник не определён)")
        + f", URL={url!r}"
    )
    raise TikTokAlreadyLoggedInError(username=username, detail=f"URL={url!r}")
...
def _is_classic_login_form_visible(page) -> bool:
    ...
    try:
        heading = page.get_by_text(
            re.compile(r"^log\s+in\s+to\s+tiktok$|^вход\s+в\s+tiktok$", re.I)
        ).first
...
def _fill_classic_login_form(page, login: str, password: str) -> None:
    ...
    if not login:
        raise RuntimeError(
            "TikTok: пустой логин для формы входа "
            "(нужен tt_login в данных учётки)."
        )
    if not password:
        raise RuntimeError(
            "TikTok: пустой пароль для формы входа "
            "(нужен tt_password или gmail_password)."
        )
    form = page.locator("form#login_form").first
    email_candidates = (
    ...
    if not filled_login:
        raise RuntimeError(
            "TikTok: не найдено поле логина на форме входа"
    ...
    if not filled_pwd:
        raise RuntimeError(
            "TikTok: не найдено поле Password на форме входа"
            + (f": {last_err!r}" if last_err else "")
        )
...
def _fill_onetap_password(page, password: str) -> None:
    ...
    if not password:
        raise RuntimeError(
            "TikTok: пустой пароль для входа в сохранённый профиль "
            "(нужен tt_password или gmail_password)."
        )
    candidates = (
    ...
    for inp in candidates:
        try:
            ...
            continue
    raise RuntimeError(
        "TikTok: не найдено поле Password на экране сохранённого профиля"
        + (f": {last_err!r}" if last_err else "")
    )
...
def _ensure_trust_device_checked(page) -> None:
    ...
    try:
        ...
        _log("TikTok: включили «Trust this device».")
    except Exception:
        pass
...
def _handle_login_2fa_challenge(page, twofa_secret: str, *, max_seconds: float = 60.0) -> None:
    ...
    if not secret:
        raise RuntimeError(
            "TikTok: требуется 2FA при входе, но tt_2fa не задан "
            "в данных учётки профиля."
        )
    _ensure_trust_device_checked(page)
    otp = get_totp_token(secret)
    _log("TikTok: вводим TOTP-код на экране 2FA входа…")
    if not _fill_login_2fa_code(page, otp):
        raise RuntimeError(
            "TikTok: не найдено поле Code на экране 2FA входа."
        )
    page.wait_for_timeout(400)
    ...
    while time.monotonic() < deadline:
        ...
        if _click_login_2fa_continue(page):
            _log("TikTok: нажали Continue после 2FA.")
            return
        page.wait_for_timeout(350)
    raise RuntimeError(
        "TikTok: не удалось нажать Continue на экране 2FA входа."
    )
...
def try_tiktok_saved_profile_login(page, password: str) -> str | None:
    """
    ...
    _log(
        "TikTok: экран сохранённого профиля"
        + (f" (@{username})" if username else "")
        + " — жмём Continue…"
    )
    if not _click_saved_profile_continue(page):
        _log("TikTok: не удалось нажать Continue на сохранённом профиле.")
        return None
    ...
    while time.monotonic() < deadline:
        ...
        if _tiktok_already_logged_in(page):
            _log("TikTok: после Continue сразу вошли без пароля.")
            return username or _extract_logged_in_username(page) or "saved_profile"
        page.wait_for_timeout(400)
    else:
        _log("TikTok: поле пароля после Continue не появилось.")
        return None

    _log("TikTok: вводим пароль на экране сохранённого профиля…")
    try:
        _fill_onetap_password(page, password)
    except Exception as e:
        _log(f"TikTok: не удалось ввести пароль: {e!r}")
        return None
    page.wait_for_timeout(400)

    if not _click_onetap_log_in(page):
        _log("TikTok: не удалось нажать Log in.")
        return None
    _log("TikTok: нажали Log in.")
    page.wait_for_timeout(1000)
    ...
    while time.monotonic() < save_deadline:
        ...
        if _is_save_login_info_screen(page):
            if _click_save_login_info(page):
                _log("TikTok: «Сохраните пароль» — нажали «Не сейчас».")
                page.wait_for_timeout(1000)
            break
        if _tiktok_already_logged_in(page):
            break
    ...
    while time.monotonic() < home_deadline:
        ...
        if _tiktok_already_logged_in(page):
            ...
            _log(f"TikTok: вход через сохранённый профиль успешен (@{uname}).")
            return uname
        page.wait_for_timeout(500)

    _log("TikTok: после Log in лента не открылась.")
    return None


def ensure_tiktok_session_relogin(
    page,
    *,
    password: str,
    twofa_secret: str = "",
    ...
    if not pwd:
        raise RuntimeError(
            "TikTok: сессия разлогинена, но нет пароля "
            "(tt_password или gmail_password) в данных учётки."
        )
    ...
    if landing and not classic:
        if not user_login:
            raise RuntimeError(
                "TikTok: экран «Войти», но нет tt_login в данных учётки."
            )
        _log("TikTok: разлогин — mobile splash без профиля, жмём «Войти»…")
        if not _click_mobile_landing_log_in(page):
            raise RuntimeError(
                "TikTok: не удалось нажать «Войти» на стартовом экране."
            )
        page.wait_for_timeout(800)
        ...
        while time.monotonic() < form_deadline:
            ...
            if _is_saved_profile_chooser_screen(page):
                return ensure_tiktok_session_relogin(
                    page,
                    password=pwd,
        ...
        if not classic:
            raise RuntimeError(
                "TikTok: после «Войти» на splash не появилась форма логина "
                f"(URL={(page.url or '')!r})."
            )
    ...
    if classic:
        if not user_login:
            raise RuntimeError(
                "TikTok: форма входа, но нет tt_login в данных учётки."
            )
        _log(
            "TikTok: разлогин — форма входа "
        ...
        while time.monotonic() < login_deadline:
            ...
            page.wait_for_timeout(350)
        if not clicked:
            raise RuntimeError("TikTok: не удалось нажать «Log in» / «Войти».")
        _log("TikTok: нажали «Log in».")
        page.wait_for_timeout(1000)
    else:
        if chooser:
            _log(
                "TikTok: разлогин — экран сохранённого профиля"
                + (f" (@{username})" if username else "")
                + " — жмём Continue…"
            )
            if not _click_saved_profile_continue(page):
                raise RuntimeError(
                    "TikTok: не удалось нажать «Продолжить» на экране "
        ...
        while time.monotonic() < deadline:
            ...
            if _tiktok_already_logged_in(page):
                ...
                _log(f"TikTok: после Continue сразу вошли (@{uname}).")
                return uname
            ...
            raise RuntimeError(
                "TikTok: после «Продолжить» не появилось поле пароля / 2FA / код почты."
            )

        if _onetap_password_visible(page):
            _log("TikTok: вводим пароль (tt_password / gmail_password)…")
            ...
            while time.monotonic() < login_deadline:
                ...
                page.wait_for_timeout(350)
            if not clicked:
                raise RuntimeError("TikTok: не удалось нажать «Войти».")
            _log("TikTok: нажали «Войти».")
            page.wait_for_timeout(1000)
    ...
    while time.monotonic() < settle_deadline:
        if _wrong_login_credentials_visible(page):
            raise RuntimeError(
                "TikTok: введены неверные данные для входа "
        ...
        try:
            ...
            _log(f"TikTok: import email handler: {e_imp!r}")
        if email_scr:
            if email_handler is None:
                raise RuntimeError(
                    "TikTok: экран кода из почты, но обработчик недоступен "
                    f"(URL={(page.url or '')!r})."
                )
        ...
        try:
            ...
            _log(f"TikTok: skip new password: {e_skip!r}")
        if dismiss_tiktok_scraping_warning_if_present(page):
            continue
        if _is_save_login_info_screen(page):
            if _click_save_login_info(page):
        ...
        if _tiktok_already_logged_in(page):
            ...
            _log(f"TikTok: re-login успешен (@{uname}).")
            return uname
    ...
    raise RuntimeError(
        "TikTok: после ввода пароля не удалось войти в аккаунт "
        f"(URL={(page.url or '')!r})."
    )
...
def _wait_signup_form(
    ...
    *,
    ...
    while time.monotonic() < deadline:
        accept_tiktok_cookie_consent_if_present(page)
        if _signup_form_visible(page):
            return
        _raise_if_already_logged_in(page)
        abort_if_tiktok_sms_image_captcha(page)
        page.wait_for_timeout(400)
    # Перед таймаутом — ещё раз: часто уже лента, а не «форма не открылась».
    _raise_if_already_logged_in(page)
    raise RuntimeError(
        f"TikTok: форма регистрации не открылась за {max_seconds:.0f} с "
        f"(URL={page.url!r})"
    )
...
def _email_confirmation_code_rejected(page) -> bool:
    """TikTok показал ошибку неверного кода из письма."""
    try:
...
def click_tiktok_terms_agree(page, *, max_seconds: float = 20.0) -> bool:
    """
    ...
    while time.monotonic() < deadline:
        ...
        if _click_by_text(page, _I_AGREE_RE, prefer_link=False):
            _log("TikTok: экран условий — нажали «I agree».")
            page.wait_for_timeout(800)
            return True
        try:
            btn = page.locator('[role="button"]').filter(has_text=_I_AGREE_RE).first
            if btn.count() > 0 and btn.is_visible(timeout=400):
                btn.click(timeout=8000)
                _log("TikTok: экран условий — нажали «I agree» (role=button).")
                page.wait_for_timeout(800)
                return True
        except Exception:
            pass
    ...
    _log("TikTok: экран условий виден, но «I agree» не нажалась.")
    return False


def accept_tiktok_terms_if_present(page, *, max_seconds: float = 20.0) -> bool:
    """Если есть экран согласия с условиями — нажать I agree. True если обработали."""
    if not _is_terms_agree_screen(page):
        return False
    _log("TikTok: экран «agree to our terms».")
    return click_tiktok_terms_agree(page, max_seconds=max_seconds)


def _is_cookie_consent_url(url: str) -> bool:
    u = (url or "").strip().lower()
    return "tiktok.com" in u and "/consent" in u
    ...
    try:
        clicked = page.evaluate(
            """() => {
                ...
                const onConsent = /tiktok\\.com.*\\/consent/i.test(
                    location.href || ''
                );
        ...
        if clicked:
            ...
            if re.search(
                ...
                _log(
                    f"TikTok: cookie consent — JS попал не в Allow "
            ...
            if not re.search(
                ...
                _log(
                    f"TikTok: cookie consent — JS click без Allow "
                    f"({clicked!r}), пропускаем."
                )
                return False
            _log(f"TikTok: cookie consent — JS click ({clicked!r})")
            return True
    except Exception as e:
        _log(f"TikTok: cookie JS click failed: {e!r}")
    return False
...
def click_tiktok_allow_all_cookies(page, *, max_seconds: float = 30.0) -> bool:
    """
    ...
    while time.monotonic() < deadline:
        ...
        if (
            ...
            if clicked_once:
                _log(f"TikTok: после cookies URL={url!r}")
            return True
        ...
        if not _cookie_allow_button_present(page):
            ...
            if not _cookie_allow_button_present(page):
                ...
                if _is_cookie_consent_url(url) or _cookie_dialog(page).count() > 0:
                    if _click_allow_all_cookies_via_js(page):
                        clicked_once = True
                        _log("TikTok: cookie consent — JS click (ожидание).")
                        page.wait_for_timeout(800)
                        continue
                page.wait_for_timeout(400)
                continue
        ...
        if _click_allow_all_cookies_button(page):
            clicked_once = True
            _log("TikTok: cookie consent — нажали «Allow all cookies».")
            ...
            if not _is_cookie_consent_url(_page_url(page)) and _cookie_consent_dismissed(
                ...
                _log(f"TikTok: cookie dialog закрыт, URL={_page_url(page)!r}")
                return True
            _log("TikTok: cookie клик был, но экран ещё виден — повторяем…")
            continue
        page.wait_for_timeout(400)

    _log("TikTok: cookie consent виден, но «Allow all cookies» не нажалась.")
    return False


def accept_tiktok_cookie_consent_if_present(
    ...
    while True:
        if _is_cookie_consent_screen(page):
            _log(
                f"TikTok: экран согласия на cookies "
                f"(URL={_page_url(page)!r})."
            )
            return click_tiktok_allow_all_cookies(page, max_seconds=max_seconds)
...
def _wait_captcha_or_code_screen(
    ...
    *,
    ...
    deadline = time.monotonic() + max_seconds
    while time.monotonic() < deadline:
        abort_if_tiktok_registration_failed(page)
        accept_tiktok_terms_if_present(page, max_seconds=8.0)
        if _is_confirmation_code_screen(page):
            _log("TikTok: экран кода подтверждения (без капчи).")
            return "code"
        if _captcha_iframe_visible(page):
            _log("TikTok: iframe капчи появился.")
            return "captcha"
        page.wait_for_timeout(500)
    raise RuntimeError(
        f"TikTok: ни капча, ни экран кода не появились за {max_seconds:.0f} с "
        f"(URL={page.url!r})"
    )
...
"""
    ...
    for frame in list(page.frames):
        try:
            ...
            continue
    if total > 0:
        _log(f"TikTok: обновил iframe капчи ({total}).")
        return True
    return False
...
def _wait_captcha_ready_after_reload(
    ...
    *,
    ...
    _log(
        f"TikTok: жду прогрузку капчи после обновления "
    ...
    while time.monotonic() < deadline:
        ...
        if _recaptcha_checkbox_ready(page):
            _log("TikTok: капча прогрузилась (чекбокс виден).")
            return True
        if _captcha_iframe_visible(page):
            ...
            if _recaptcha_checkbox_ready(page) or _captcha_iframe_visible(page):
                _log("TikTok: капча прогрузилась (iframe виден).")
                return True
        page.wait_for_timeout(400)
    _log("TikTok: капча после обновления не прогрузилась за отведённое время.")
    return False
...
def _nudge_captcha_click(page) -> bool:
    ...
    if reloaded:
        if not _wait_captcha_ready_after_reload(page):
            ...
            _log("TikTok: кликаю по капче без подтверждения прогрузки…")
    else:
        _log("TikTok: iframe капчи для обновления не найден — кликаю как есть.")
    ...
    try:
        for frame in list(page.frames):
            ...
            try:
                ...
                _log("TikTok: клик по чекбоксу reCAPTCHA (nudge).")
                return True
            except Exception:
                continue
    except Exception:
    ...
    try:
        ...
        box = fl.locator(_RECAPTCHA_ANCHOR_SEL).first
        box.click(timeout=2500, force=True)
        _log("TikTok: клик по reCAPTCHA через frame_locator (nudge).")
        return True
    except Exception:
        pass
    ...
    try:
        ...
        if iframe.count() > 0 and iframe.is_visible(timeout=500):
            ...
            _log("TikTok: клик по iframe капчи (nudge).")
            return True
    except Exception:
        pass
    ...
    try:
        for sel in (
            ...
            _log(f"TikTok: клик по AntiCaptcha UI ({sel}) (nudge).")
            return True
    except Exception:
        pass
...
def _wait_anticaptcha_extension_solve(
    ...
    *,
    ...
    last_log = 0.0
    _log(
        f"TikTok: жду расширение AntiCaptcha до {max_seconds:.0f} с "
        "(бейдж .antigate_solver.solved / «Далее» / экран кода)…"
    )
    while time.monotonic() < deadline:
        accept_tiktok_terms_if_present(page, max_seconds=5.0)
        if _is_confirmation_code_screen(page):
            _log("TikTok: расширение — уже экран кода.")
            return True
        if _extension_captcha_solved_visible(page):
            _log("TikTok: расширение отметило капчу как solved.")
            if _next_button_enabled(page) and _click_tiktok_next(page):
                return True
            if _is_confirmation_code_screen(page):
                return True
        ...
        if _captcha_passed_locally(page) or _next_button_enabled(page):
            if _click_tiktok_next(page) or _is_confirmation_code_screen(page):
                _log("TikTok: капча пройдена (Next активна) — вероятно расширение.")
                return True
        now = time.monotonic()
        if now - last_log >= _MANUAL_CAPTCHA_LOG_EVERY_S:
            _log(
                f"TikTok: всё ещё жду расширение… "
                f"({int(now - (deadline - max_seconds))} с)."
            )
            last_log = now
        page.wait_for_timeout(800)
    _log("TikTok: расширение не решило капчу за отведённое время.")
    return False


def wait_tiktok_manual_captcha(
    page,
    *,
    ...
    """
    if _is_confirmation_code_screen(page):
        _log("TikTok: капча не нужна — уже экран кода.")
        return

    _log(
        "TikTok: капча — жду ручного прохождения "
    ...
    if callable(on_manual_captcha):
        try:
            ...
            _log(f"TikTok: on_manual_captcha: {e!r}")
    ...
    while True:
        if _is_confirmation_code_screen(page):
            _log("TikTok: во время ожидания капчи открылся экран кода.")
            return
        if _next_button_enabled(page):
            _log("TikTok: «Далее» активна — капча пройдена вручную.")
            break
        if _captcha_passed_locally(page):
            _log("TikTok: капча пройдена (токен/Next) во время ожидания.")
            break
        if _extension_captcha_solved_visible(page):
            _log("TikTok: расширение отметило solved во время ручного ожидания.")
            break
        now = time.monotonic()
        waited = now - started
        if waited >= _MANUAL_CAPTCHA_NUDGE_AFTER_S and (
            ...
            _log(
                f"TikTok: ручная капча без прогресса {int(waited)} с — "
            ...
            if _nudge_captcha_click(page):
                ...
                if _captcha_passed_locally(page) or _extension_captcha_solved_visible(
                    ...
                    _log("TikTok: после nudge капча выглядит пройденной.")
                    break
                if _is_confirmation_code_screen(page):
                    _log("TikTok: после nudge уже экран кода.")
                    return
            else:
                _log("TikTok: nudge — элемент капчи для клика не найден.")
            last_nudge = time.monotonic()
        if now - last_log >= _MANUAL_CAPTCHA_LOG_EVERY_S:
            _log(
                f"TikTok: всё ещё жду ручную капчу… ({int(waited)} с). "
    ...
    for _ in range(8):
        if _is_confirmation_code_screen(page):
            _log("TikTok: экран кода после прохождения капчи.")
            return
        if _next_button_enabled(page):
            break
        page.wait_for_timeout(500)
    ...
    if _click_by_text(page, _NEXT_RE, prefer_link=False):
        _log("TikTok: нажали «Далее» после капчи.")
        page.wait_for_timeout(800)
        return
    ...
    try:
        page.get_by_role("button", name=_NEXT_RE).first.click(timeout=5000)
        _log("TikTok: нажали «Далее» (role=button) после капчи.")
        page.wait_for_timeout(800)
        return
    except Exception:
        pass
    if _is_confirmation_code_screen(page):
        return
    raise RuntimeError(
        f"TikTok: «Далее» стала активной, но клик не удался (URL={page.url!r})"
    )


def wait_tiktok_after_signup(
    page,
    *,
    on_manual_captcha=None,
) -> None:
    ...
    page.wait_for_timeout(600)
    abort_if_tiktok_registration_failed(page)
    accept_tiktok_terms_if_present(page, max_seconds=15.0)
    abort_if_tiktok_registration_failed(page)
    ...
    if settle_ms > 0:
        _log(
            f"TikTok: жду {_CAPTCHA_IFRAME_SETTLE_S:.0f} с после iframe "
            "перед решением капчи…"
        )
        page.wait_for_timeout(settle_ms)
        abort_if_tiktok_registration_failed(page)
        if _is_confirmation_code_screen(page):
            _log("TikTok: за время паузы уже экран кода.")
            return
    ...
    if _wait_anticaptcha_extension_solve(page):
        ...
        if _is_confirmation_code_screen(page):
            return
        wait_tiktok_confirmation_code_screen(page)
        return

    wait_tiktok_manual_captcha(
        page,
...
def wait_tiktok_confirmation_code_screen(
    page,
    *,
    ...
    while time.monotonic() < deadline:
        accept_tiktok_terms_if_present(page, max_seconds=8.0)
        if _is_confirmation_code_screen(page):
            _log("TikTok: экран кода подтверждения.")
            return
        page.wait_for_timeout(500)
    raise RuntimeError(
        f"TikTok: экран кода подтверждения не появился за {max_seconds:.0f} с "
        f"(URL={page.url!r})"
    )


def fill_tiktok_confirmation_code(page, code: str) -> None:
    """Ввести код в поле «Код подтверждения» / Bloks «Введите код»."""
    code = (code or "").strip()
    if not re.fullmatch(r"\d{6,8}", code):
        raise RuntimeError(f"TikTok: ожидался 6–8-значный код, получили {code!r}")
    ...
    try:
        ...
        _log(f"TikTok: fill по label не сработал: {e!r}")
    ...
    if not filled:
        try:
            ...
            raise RuntimeError(
                f"TikTok: не удалось ввести код подтверждения: {e!r}"
            ) from e

    if not filled:
        raise RuntimeError("TikTok: поле кода подтверждения не найдено.")
    _log(f"TikTok: ввели код {code}.")


def click_tiktok_continue_after_code(
    page,
    *,
    ...
    while time.monotonic() < deadline:
        ...
        try:
            ...
            if btn.count() > 0 and btn.is_visible(timeout=300):
                ...
                if not disabled:
                    ...
                    _log("TikTok: нажали Continue (aria-label).")
                    page.wait_for_timeout(800)
                    return
        except Exception:
            pass
        page.wait_for_timeout(400)
    raise RuntimeError(
        f"TikTok: кнопка «Продолжить» не найдена за {max_seconds:.0f} с "
        f"(URL={page.url!r})"
    )
...
def _tiktok_home_ready(page, username: str) -> bool:
    ...
    try:
        ...
        pass
    try:
        link = page.locator("a[href*='tiktok.com/']").filter(has_text=name_re).first
    ...
    try:
        ...
        if text.count() > 0 and text.is_visible(timeout=150):
            if "tiktok.com" in url and (
                "/accounts/" not in url
                or "onetap" in url
                or url.rstrip("/").endswith("tiktok.com")
            ):
                return True
    except Exception:
        pass
...
def click_tiktok_human_confirm_continue(page, *, max_seconds: float = 30.0) -> bool:
        ...
        try:
            ...
            if btn.count() > 0 and btn.is_visible(timeout=400):
                ...
                if not disabled and btn.get_attribute("disabled") is None:
                    btn.click(timeout=8000)
                    _log("TikTok: Confirm you're human — нажали Continue.")
                    page.wait_for_timeout(1000)
                    return True
        except Exception:
            pass
        if _click_by_text(page, _CONTINUE_RE, prefer_link=False):
            _log("TikTok: Confirm you're human — нажали Continue (текст).")
            page.wait_for_timeout(1000)
            return True
        page.wait_for_timeout(400)
    return False
...
def resolve_tiktok_image_captcha(page) -> None:
    """Image/SMS captcha → тег и остановка авторега (не решаем)."""
    abort_if_tiktok_sms_image_captcha(page)


def handle_tiktok_human_confirmation(
    page,
    username: str,
    *,
    ...
    if _tiktok_home_ready(page, username):
        return
    ...
    _log(
        "TikTok: экран Confirm you're human / suspended не появился — "
        "идём к проверке главной."
    )


def wait_tiktok_home_with_username(
    page,
    username: str,
    *,
    ...
    username = (username or "").strip().lstrip("@")
    if not username:
        raise RuntimeError("TikTok: пустой username для проверки главной.")
    ...
    while time.monotonic() < deadline:
        ...
        try:
            ...
            if link.count() > 0 and link.is_visible(timeout=300):
                _log(f"TikTok: главная — профиль /{username}/.")
                return
        except Exception:
            pass
        try:
            link = page.locator("a[href*='tiktok.com/']").filter(has_text=name_re).first
            if link.count() > 0 and link.is_visible(timeout=200):
                _log(f"TikTok: главная — ссылка с ником {username!r}.")
                return
        except Exception:
            pass
        try:
            ...
            if text.count() > 0 and text.is_visible(timeout=200):
                ...
                if "emailsignup" not in url and "accounts/signup" not in url:
                    if "tiktok.com" in url and (
                        ...
                        _log(f"TikTok: главная — видим ник {username!r}.")
                        return
        except Exception:
            pass
        try:
            ...
            for i in range(min(any_href.count(), 8)):
                ...
                if href_re.search(href.split("?")[0]) and el.is_visible(timeout=150):
                    _log(f"TikTok: главная — href={href!r}.")
                    return
        except Exception:
            pass
        page.wait_for_timeout(1000)
    ...
    raise RuntimeError(
        f"TikTok: после кода не дождались главной с ником {username!r} "
...
def complete_tiktok_email_confirmation(
    ig_page,
    gmail_page,
    username: str,
) -> None:
    ...
    """
    from zaliver.tiktok_upload.gmail_confirmation_code import (
        fetch_tiktok_confirmation_code_from_gmail,
    )

    wait_tiktok_confirmation_code_screen(ig_page)
    # Дать TikTok время отправить письмо, прежде чем идти в Gmail.
    _log("TikTok: экран кода — ждём 3 с перед переходом на почту…")
    ig_page.wait_for_timeout(3000)
    used_codes: set[str] = set()
    last_code = ""

    for attempt in range(1, _EMAIL_CODE_ATTEMPTS + 1):
        code = fetch_tiktok_confirmation_code_from_gmail(
            gmail_page,
        ...
        if attempt > 1 and _left_email_confirmation_screen(ig_page, username):
            _log(
                "TikTok: пока ходили в Gmail, экран почтового кода уже сменился — "
                "новое письмо не нужно."
            )
            break
        wait_tiktok_confirmation_code_screen(ig_page, max_seconds=30.0)
        fill_tiktok_confirmation_code(ig_page, code)
        click_tiktok_continue_after_code(ig_page)
        ...
        if _left_email_confirmation_screen(ig_page, username):
            _log(
                f"TikTok: код {code} принят — экран почтового кода сменился "
                f"(URL={ig_page.url!r})."
            )
            break
        ...
        if not rejected and not still_email_code:
            ...
            _log(
                f"TikTok: после кода {code} больше не экран почтового кода "
                f"(URL={ig_page.url!r}) — не ждём новое письмо."
            )
            break

        _log(
            f"TikTok: после кода {code} "
            + (
                "код отклонён"
                if rejected
        ...
        raise RuntimeError(
            f"TikTok: после {_EMAIL_CODE_ATTEMPTS} кодов из почты "
    ...
    handle_tiktok_human_confirmation(ig_page, username)
    wait_tiktok_home_with_username(ig_page, username)
    _log(f"TikTok: регистрация подтверждена, username={username!r}.")
...
def _is_tiktok_url(url: str) -> bool:
    u = (url or "").strip().lower()
    return "tiktok.com" in u and not u.startswith("about:")


def _is_chrome_net_error_url(url: str) -> bool:
    u = (url or "").strip().lower()
...
def _navigate_page_to(
    page, url: str, *, label: str = "TikTok", keep_in_background: bool = False
) -> None:
    ...
    for name, action in strategies:
        try:
            ...
            # вкладка TikTok выходит на передний план, а возврат на YouTube
            ...
            page.wait_for_timeout(800)
            cur = _page_url(page)
            if _is_chrome_net_error_url(cur):
                _log(
                    f"{label}: сеть не поднялась ({cur!r}) — "
                    "ждём прокси и пробуем снова…"
                )
                try:
            ...
            if _is_tiktok_url(cur):
                ...
                return
            if cur and cur.lower() not in ("about:blank", "about:srcdoc"):
            ...
            if "ERR_PROXY" in err_s or "PROXY_CONNECTION" in err_s:
                try:
                    page.wait_for_timeout(2500)
                except Exception:
                    time.sleep(2.5)
...
def open_tiktok_signup_tab(
    gmail_page,
    *,
    password: str = "",
):
    """Вторая вкладка: TikTok → сохранённый профиль / «Создать новый аккаунт»."""
    context = gmail_page.context
    _log(f"TikTok: открываем вкладку {TIKTOK_URL}")

    ig_page = None
    # Сначала Playwright new_page + goto — без лишней about:blank от window.open.
    try:
        ig_page = context.new_page()
        _navigate_page_to(ig_page, TIKTOK_URL)
    except Exception as e:
        _log(f"TikTok: new_page не сработал ({e!r}), пробуем window.open…")
        failed = ig_page
        ig_page = None
        ...
        try:
            ...
            try:
                ...
                _log(f"TikTok: wait после window.open: {e2!r}")
            ig_page.wait_for_timeout(1000)
            cur = _page_url(ig_page)
            _log(f"TikTok: вкладка через window.open, URL={cur!r}")
            if not _is_tiktok_url(cur):
                _navigate_page_to(ig_page, TIKTOK_URL)
        except Exception as e2:
            raise RuntimeError(
                f"TikTok: не удалось открыть вкладку "
                f"(new_page: {e!r}; window.open: {e2!r})"
            ) from e2

    if ig_page is None:
        raise RuntimeError("TikTok: не удалось создать вкладку.")
    ...
    accept_tiktok_cookie_consent_if_present(ig_page, appear_seconds=12.0)

    # Сразу на suspended — циферная капча: стоп с тегом SMS.
    abort_if_tiktok_sms_image_captcha(ig_page)
    ...
    if pwd and _is_saved_profile_chooser_screen(ig_page):
        uname = try_tiktok_saved_profile_login(ig_page, pwd)
        if uname:
            raise TikTokAlreadyLoggedInError(
                username=uname,
                detail="вход через сохранённый профиль (Continue)",
            )
        _log(
            "TikTok: вход через сохранённый профиль не удался — "
    ...
    if not _signup_form_visible(ig_page):
        if not _click_by_text(ig_page, _CREATE_ACCOUNT_RE, prefer_link=True):
            ...
            signup = "https://www.tiktok.com/accounts/emailsignup/"
            _log(f"TikTok: кнопка создания не найдена — goto {signup}")
            ...
            while time.monotonic() < settle:
                if _signup_form_visible(ig_page):
                    break
                if _tiktok_already_logged_in(ig_page):
                    _raise_if_already_logged_in(ig_page)
                try:
                    ig_page.wait_for_timeout(500)
                except Exception:
            ...
            _log("TikTok: нажали «Создать новый аккаунт».")

    _wait_signup_form(ig_page)
    return ig_page


def fill_tiktok_signup_form(
    page,
    ...
    if not email:
        raise RuntimeError("TikTok: пустой yt_login (email) для регистрации.")
    if not password:
        raise RuntimeError("TikTok: пустой yt_password для регистрации.")

    local = email_local_part(email)
    if not local:
        raise RuntimeError(f"TikTok: не удалось взять локальную часть email {email!r}")

    _log(f"TikTok: email={email!r}, username={local!r} (full name пропускаем)")

    # Cookie-диалог может всплыть уже на форме signup.
    accept_tiktok_cookie_consent_if_present(page, appear_seconds=5.0)
    ...
    _log("TikTok: ждём 5 с проверку имени пользователя…")
    page.wait_for_timeout(5000)
    if _username_taken_visible(page):
        suffix = random.randint(1, 1000)
        username = f"{local}{suffix}"
        _log(f"TikTok: имя занято — пробуем {username!r}")
        _fill_labeled_input(
            page,
            ["Имя пользователя", "Username"],
            username,
        ...
        if _username_taken_visible(page):
            suffix = random.randint(1, 1000)
            username = f"{local}{suffix}"
            _log(f"TikTok: снова занято — пробуем {username!r}")
            _fill_labeled_input(
                page,
                ["Имя пользователя", "Username"],
                username,
    ...
    if not _click_by_text(page, _SUBMIT_RE, prefer_link=False):
        ...
        try:
            ...
            raise RuntimeError(
                f"TikTok: не нашли кнопку «Отправить»: {e!r}"
            ) from e
    _log("TikTok: нажали «Отправить».")
...
@tiktok_entrypoint
def run_tiktok_registration_after_gmail(
    gmail_page,
    ...
    *,
    ...
    """
    Gmail уже открыт → TikTok signup → расширение AntiCaptcha / ручная капча →
    код из почты. Возвращает зарегистрированный username.

    Если в профиле уже выполнен вход в TikTok — считаем успехом
    (новый аккаунт создавать не нужно).
    """
    if credentials is None:
        raise RuntimeError("TikTok: нет credentials профиля (yt_login / yt_password).")
    try:
        ig_page = open_tiktok_signup_tab(
            gmail_page,
            password=(credentials.password or "").strip(),
        )
    except TikTokAlreadyLoggedInError as e:
        username = (e.username or "").strip().lstrip("@")
        if not username:
            username = email_local_part(credentials.email or "") or "already_logged_in"
        _log(
            "TikTok: аккаунт уже доступен"
            + (f" (@{username})" if username else "")
            + " — регистрацию пропускаем, успех."
        )
        return username
    try:
        username = fill_tiktok_signup_form(ig_page, credentials)
        wait_tiktok_after_signup(
            ig_page,
            on_manual_captcha=on_manual_captcha,
        )
        complete_tiktok_email_confirmation(
            ig_page,
            gmail_page,
            username,
        )
