...
ACCOUNTS_CENTER_URL = (
    "https://www.tiktok.com/?entry_point=app_settings"
)
PASSWORD_SECURITY_URL = "https://www.tiktok.com/password_and_security/"
...
KEEP_PROFILE_OPEN_AFTER_TT_2FA = False
...
class TikTokLoginRequiredError(RuntimeError):
    """Редирект на https://www.tiktok.com/accounts/login/."""


def _log(message: str) -> None:
    emit_tiktok_log(message, tag="[tiktok-2fa]")


def _is_accounts_login(page) -> bool:
    """URL /accounts/login — нет сессии TikTok."""
    url = ""
    try:
...
def _wait_leave_about_blank(page, *, max_seconds: float = _BLANK_SETTLE_S) -> str:
    ...
    while time.monotonic() < deadline:
        ...
        if cur and low not in ("about:blank", "about:srcdoc", ""):
            _log(f"TikTok 2FA: вкладка ушла с about:blank → {cur!r}")
            return cur
        try:
...
def _ensure_accounts_center_open(page) -> None:
    ...
    if low0 in ("about:blank", "about:srcdoc", ""):
        _log(
            "TikTok 2FA: вкладка ещё about:blank — ждём старт браузера "
    ...
    if _is_accounts_center_url(url0):
        _log(f"TikTok 2FA: уже в Accounts Center (URL={url0!r}).")
        return

    _log(
        f"TikTok 2FA: текущий URL={url0!r} — открываем Accounts Center…"
    )
    _navigate_page_to(page, ACCOUNTS_CENTER_URL, label="TikTok 2FA")


def _raise_if_tiktok_session_abort(page) -> None:
    """
    suspended / login → стоп с ошибкой (тег + закрытие профиля).
    """
    url = _page_url(page)
    if _is_accounts_suspended(page):
        _log(f"TikTok 2FA: редирект на suspended — стоп (URL={url!r}).")
        raise TikTokAccountSuspendedError(
            "TikTok 2FA: аккаунт на /accounts/suspended "
            f"(URL={url!r})."
        )
    if _is_accounts_login(page):
        _log(f"TikTok 2FA: редирект на login — стоп (URL={url!r}).")
        raise TikTokLoginRequiredError(
            "TikTok 2FA: нет входа, /accounts/login "
            f"(URL={url!r})."
        )


# Старое имя — алиас для вызовов в потоке.
_raise_if_tiktok_suspended = _raise_if_tiktok_session_abort


def _wait_raise_if_tiktok_suspended(page, *, settle_s: float = 4.0) -> None:
    """Подождать возможный редирект на suspended/login после навигации."""
    deadline = time.monotonic() + max(0.5, float(settle_s))
    while True:
        _raise_if_tiktok_session_abort(page)
...
def _click_continue(page) -> bool:
    if _click_role_button(page, _CONTINUE_RE):
        _log("TikTok 2FA: нажали Continue/Next.")
        page.wait_for_timeout(900)
        return True
    if _click_by_text(page, _CONTINUE_RE, prefer_link=False):
        _log("TikTok 2FA: нажали Continue (по тексту).")
        page.wait_for_timeout(900)
        return True
    return False
...
def _click_enter_code(page) -> bool:
    if _click_role_button(page, _ENTER_CODE_BTN_RE):
        _log("TikTok 2FA: нажали Enter code.")
        page.wait_for_timeout(900)
        return True
    if _click_by_text(page, _ENTER_CODE_BTN_RE, prefer_link=False):
        _log("TikTok 2FA: нажали Enter code (по тексту).")
        page.wait_for_timeout(900)
        return True
    return False
    ...
    try:
        ...
        if _visible(btn, timeout=800):
            ...
            if disabled in ("true", "1"):
                return False
            btn.click(timeout=10_000)
            _log("TikTok 2FA: нажали Next (locator).")
            page.wait_for_timeout(900)
            return True
    except Exception:
        pass
...
def _extract_secret_from_page(page) -> str:
  ...
  return out.slice(0, 120);
    ...
    if _click_role_button(page, _COPY_KEY_RE) or _click_by_text(
        ...
        _log("TikTok 2FA: нажали Copy key.")
  ...
  return '';
            ...
            _log(f"TikTok 2FA: clipboard недоступен: {e!r}")
...
def _reveal_manual_key_if_needed(page) -> None:
    if _click_role_button(page, _CANT_SCAN_RE) or _click_by_text(
        ...
        _log("TikTok 2FA: открыли ручной ввод ключа (Can't scan).")
...
def _wait_for_generated_secret(
    ...
    *,
    ...
    _log("TikTok 2FA: ждём генерацию ключа…")
    while time.monotonic() < deadline:
...
def _wait_for_code_input(
    ...
    *,
    ...
    _log("TikTok 2FA: ждём окно ввода кода…")
    while time.monotonic() < deadline:
        _guard_step(page, login_credentials=login_credentials, deadline=deadline)
        try:
...
def _click_skip_new_password(page) -> bool:
    for sel in (
        ...
        try:
            ...
            if btn.count() > 0 and btn.is_visible(timeout=400):
                ...
                _log("TikTok: «Создайте новый пароль» — нажали «Пропустить».")
                return True
        except Exception:
            continue
    try:
    ...
    if _click_by_text(page, _SKIP_RE, timeout_ms=2500):
        _log("TikTok: «Создайте новый пароль» — нажали «Пропустить» (текст).")
        return True
    return False
...
def _fill_meta_email_code(page, code: str) -> bool:
    ...
    for target in targets:
        try:
            ...
            _log("TikTok 2FA: ввели код из письма Meta.")
            page.wait_for_timeout(400)
            return True
        except Exception as e:
            _log(f"TikTok 2FA: fill email code failed: {e!r}")
            continue
    return False
    ...
    while time.monotonic() < deadline:
        try:
            ...
            if btn.count() > 0:
                ...
                if disabled not in ("true", "1"):
                    ...
                    _log("TikTok: нажали «Продолжить» (aria-label).")
                    page.wait_for_timeout(900)
                    return True
        except Exception:
            pass
        try:
            ...
            if _visible(btn, timeout=600):
                ...
                if disabled not in ("true", "1"):
                    btn.click(timeout=10_000)
                    _log("TikTok 2FA: нажали Continue.")
                    page.wait_for_timeout(900)
                    return True
        except Exception:
            pass
...
def _open_gmail_tab(accounts_page, login_credentials=None):
    from zaliver.tiktok_upload.gmail_availability import (
    ...
    try:
        ...
        _log("TikTok: открыли вкладку Gmail (desktop emulation)…")
...
def _handle_email_verification_if_needed(
    ...
    *,
    ...
    _log(
        "TikTok: экран «Проверьте электронную почту» — "
    ...
    try:
        for attempt in range(1, 4):
            ...
            try:
                ...
                _log(
                    f"TikTok: письмо Meta не найдено ({e_meta!r}) — "
                    "пробуем код TikTok (6 цифр)…"
                )
            ...
            page.wait_for_timeout(500)
            if not _email_check_screen_visible(page):
                _log("TikTok: экран почты уже сменился.")
                return True
            if not _fill_meta_email_code(page, code):
                raise RuntimeError(
                    "TikTok: не удалось ввести код из письма "
            ...
            if not _email_check_screen_visible(page):
                ...
                return True
        raise RuntimeError(
            "TikTok: код из письма не принят "
            f"(URL={_page_url(page)!r})."
        )
    finally:
        try:
            if gmail is not None and not gmail.is_closed():
                gmail.close()
                _log("TikTok: вкладка Gmail закрыта.")
        except Exception:
            pass
        try:
            page.bring_to_front()
...
def _guard_step(
    ...
    *,
    ...
    """
    _raise_if_tiktok_suspended(page)
...
def _click_extra_protection_get_started(page) -> bool:
    ...
    if _click_role_button(page, _GET_STARTED_RE):
        _log("TikTok 2FA: интро 2FA — нажали Get started.")
        page.wait_for_timeout(1000)
        return True
    if _click_by_text(page, _GET_STARTED_RE, prefer_link=False):
        _log("TikTok 2FA: интро 2FA — нажали Get started (по тексту).")
        page.wait_for_timeout(1000)
        return True
    try:
        ...
        if _visible(btn, timeout=800):
            btn.click(timeout=10_000)
            _log("TikTok 2FA: интро 2FA — нажали Get started (locator).")
            page.wait_for_timeout(1000)
            return True
    except Exception:
        pass
...
def _select_tiktok_account_if_needed(page) -> None:
    """Если есть выбор аккаунта — кликаем первый с меткой TikTok."""
    try:
    ...
    # Предпочитаем кнопку, в которой есть «TikTok».
    candidates = [
        page.locator('[role="button"]').filter(
            has_text=re.compile(r"TikTok", re.I)
        ),
        page.get_by_role("button").filter(has_text=re.compile(r"TikTok", re.I)),
    ]
    for loc in candidates:
        try:
            ...
            if _visible(first, timeout=1200):
                ...
                _log("TikTok 2FA: выбрали аккаунт TikTok.")
                page.wait_for_timeout(1000)
                return
        except Exception:
            continue
        ...
        if _visible(dialog, timeout=800):
            ...
            for i in range(min(btns.count(), 8)):
                ...
                if _GET_STARTED_RE.search(text):
                    continue
                if "tiktok" in text or (text and "close" not in label):
                    btn.click(timeout=10_000)
                    _log("TikTok 2FA: выбрали первый аккаунт в диалоге.")
                    page.wait_for_timeout(1000)
                    return
    except Exception:
        pass
    ...
    try:
        ...
        if radio.count() > 0:
            ...
            if not checked:
                ...
                _log("TikTok 2FA: выбрали Authentication app.")
                page.wait_for_timeout(400)
            else:
                _log("TikTok 2FA: Authentication app уже выбран.")
            return
    except Exception:
        pass
    try:
        ...
        if _visible(label, timeout=800):
            ...
            _log("TikTok 2FA: клик по label Authentication app.")
            page.wait_for_timeout(400)
    except Exception:
        pass
...
def _fill_totp_code(page, code: str, inp=None) -> bool:
    ...
    for target in targets:
        try:
            ...
            target.fill("")
            target.fill(code, timeout=10_000)
            _log("TikTok 2FA: ввели OTP-код.")
            page.wait_for_timeout(400)
            return True
        except Exception as e:
            _log(f"TikTok 2FA: fill OTP attempt failed: {e!r}")
            continue
    return False
...
def _wait_next_totp_code(secret: str, previous: str = "") -> str:
    ...
    while time.monotonic() < deadline:
        ...
        if cur != prev:
            time.sleep(0.35)
            _log("TikTok 2FA: новый OTP готов.")
            return cur
        time.sleep(0.35)
...
def _submit_totp_with_retries(
    ...
    *,
    ...
    for attempt in range(1, max(1, int(max_attempts)) + 1):
        ...
        last_otp = otp
        _log(f"TikTok 2FA: попытка OTP {attempt}/{max_attempts}…")
        if not _fill_totp_code(page, otp):
            raise RuntimeError(
                "TikTok 2FA: не удалось ввести OTP-код "
                f"(URL={_page_url(page)!r})."
            )
        if not _click_next_after_code(
            ...
            raise RuntimeError(
                "TikTok 2FA: не удалось нажать Next после ввода кода "
                f"(URL={_page_url(page)!r})."
            )
        ...
        while time.monotonic() < outcome_deadline:
            ...
            if _success_on_visible(page):
                _log("TikTok 2FA: OTP принят.")
                return
            if _wrong_code_visible(page):
                _log(
                    "TikTok 2FA: код неверный "
            ...
            if _success_on_visible(page):
                return
            if _wrong_code_visible(page):
                _log("TikTok 2FA: код неверный — повтор с новым OTP…")
                last_otp = otp
                continue
            raise RuntimeError(
                "TikTok 2FA: после Next нет ни успеха, ни ошибки кода "
                f"(URL={_page_url(page)!r})."
            )
    ...
    raise RuntimeError(
        f"TikTok 2FA: OTP не принят после {max_attempts} попыток "
        f"(URL={_page_url(page)!r})."
    )
...
def _wait_success_and_click_done(
    ...
    *,
    ...
    """
    _log("TikTok 2FA: ждём экран успешного включения…")
    seen = False
    while time.monotonic() < deadline:
        _guard_step(page, login_credentials=login_credentials, deadline=deadline)
        try:
    ...
    if not seen:
        raise RuntimeError(
            "TikTok 2FA: не дождались экрана "
            "«Two-factor authentication is on» "
            f"(URL={_page_url(page)!r})."
        )
    _log("TikTok 2FA: 2FA успешно включена — жмём Done.")
    ...
    if not clicked:
        raise RuntimeError(
            "TikTok 2FA: не найдена кнопка Done на экране успеха "
            f"(URL={_page_url(page)!r})."
        )

    _log("TikTok 2FA: Done нажали, ждём 1 с перед закрытием профиля…")
    page.wait_for_timeout(1000)
...
def _finish_already_enabled(page) -> str:
    ...
    _log("TikTok 2FA: уже включена — ставим успешный статус.")
    try:
        close = page.get_by_role("button", name=_CLOSE_BTN_RE).first
        if _visible(close, timeout=800):
            close.click(timeout=5000)
            _log("TikTok 2FA: закрыли диалог (Close).")
    except Exception:
        pass
    page.wait_for_timeout(1000)
    _log(f"TikTok 2FA: уже включена, URL={_page_url(page)!r}.")
    return ""


@tiktok_entrypoint
def setup_tiktok_totp_2fa(
    page,
    *,
    ...
    try:
        ...
        if not _is_tiktok_url(url0) and "accountscenter" not in url0:
            # Могли открыть about:blank — сначала главная TikTok.
            if url0 in ("about:blank", "about:srcdoc", ""):
                _navigate_page_to(page, TIKTOK_URL, label="TikTok 2FA")
                accept_tiktok_cookie_consent_if_present(page, appear_seconds=6.0)
                # Дать UI отрисовать chooser / форму / ленту.
                settle = time.monotonic() + 8.0
                while time.monotonic() < settle:
                    if _needs_relogin() or _tiktok_already_logged_in(page):
                        break
                    page.wait_for_timeout(350)
        if _needs_relogin():
            ensure_tiktok_session_relogin(
        ...
        if _needs_relogin():
            raise
        _log(f"TikTok 2FA: pre-check session: {e!r}")

    _log(f"TikTok 2FA: открываем Accounts Center ({ACCOUNTS_CENTER_URL})")
    _ensure_accounts_center_open(page)
    _wait_raise_if_tiktok_suspended(page, settle_s=5.0)
    ...
    if _click_by_text(page, _PASSWORD_SECURITY_RE, prefer_link=True):
        opened_ps = True
        _log("TikTok 2FA: клик Password and security.")
    else:
        try:
            ...
            if _visible(link, timeout=1500):
                ...
                opened_ps = True
                _log("TikTok 2FA: клик href password_and_security.")
        except Exception:
            pass
    if not opened_ps:
        _log("TikTok 2FA: прямая навигация на password_and_security…")
        _navigate_page_to(page, PASSWORD_SECURITY_URL, label="TikTok 2FA")
    page.wait_for_timeout(1000)
    _wait_raise_if_tiktok_suspended(page, settle_s=4.0)
    _g()
    ...
    if opened_2fa:
        _log("TikTok 2FA: открыли Two-factor authentication.")
        page.wait_for_timeout(1200)
        _wait_raise_if_tiktok_suspended(page, settle_s=3.0)
        _g()
        ...
        if not in_flow:
            raise RuntimeError(
                "TikTok 2FA: не найдена кнопка Two-factor authentication "
                f"(URL={_page_url(page)!r})."
            )
        _log(
            "TikTok 2FA: пункт Two-factor не нужен — уже в мастере "
            f"(URL={_page_url(page)!r})."
        )
    ...
    while time.monotonic() < method_deadline:
        ...
        try:
            ...
            pass
        _select_tiktok_account_if_needed(page)
        page.wait_for_timeout(400)
    ...
    if not _click_continue(page):
        ...
        if _g() and _click_continue(page):
            ...
            raise RuntimeError(
                "TikTok 2FA: не удалось нажать Continue после выбора Authentication app "
                f"(URL={_page_url(page)!r})."
            )
    _g()

    # Ждём генерацию ключа («SBIC D4C2 UQB7 …»), убираем пробелы → tt_2fa
    ...
    if not secret or len(secret) < 16:
        raise RuntimeError(
            "TikTok 2FA: не дождались сгенерированного ключа "
            f"(URL={_page_url(page)!r})."
        )

    _log(f"TikTok 2FA: секрет получен (len={len(secret)}), сохраняем в tt_2fa…")
    if on_secret is not None:
        try:
            on_secret(secret)
        except Exception as e:
            _log(f"TikTok 2FA: on_secret callback failed: {e!r}")
            raise
    ...
    if not _click_enter_code(page):
        ...
        if not clicked:
            raise RuntimeError(
                "TikTok 2FA: не найдена кнопка Enter code "
                f"(URL={_page_url(page)!r})."
            )
    ...
    if code_inp is None:
        raise RuntimeError(
            "TikTok 2FA: не дождались окна ввода кода "
            f"(URL={_page_url(page)!r})."
        )
    ...
    _log(f"TikTok 2FA: подключение завершено, URL={_page_url(page)!r}.")
    return secret
