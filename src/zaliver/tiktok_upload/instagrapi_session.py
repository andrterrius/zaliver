...
class InstagrapiSessionError(RuntimeError):
    """Не удалось получить рабочую сессию TikTok для чекера."""


def instagrapi_sessions_dir() -> Path:
    d = zaliver_app_data_dir() / "tiktok_sessions"
...
def normalize_tiktok_sessionid(raw: str) -> str:
...
def _login_with_password(
    ...
    *,
    ...
    if not user or not pwd:
        raise InstagrapiSessionError("Пустой логин или пароль TikTok.")
    ...
    try:
        ...
        if secret:
            try:
                ...
                pass
        raise InstagrapiSessionError(f"Логин TikTok не удался: {e}") from e
...
def _sessionid_from_loaded_client(cl: Any) -> str:
    ...
    if isinstance(cookies, dict):
        sid = normalize_tiktok_sessionid(str(cookies.get("sessionid") or ""))
        if sid:
            return sid
    auth = settings.get("authorization_data")
    if isinstance(auth, dict):
        sid = normalize_tiktok_sessionid(str(auth.get("sessionid") or ""))
        if sid:
            return sid
    try:
        sid = normalize_tiktok_sessionid(str(getattr(cl, "sessionid", "") or ""))
        if sid:
            return sid
    except Exception:
        pass
...
def _login_by_sessionid(cl: Any, sessionid: str) -> None:
    sid = normalize_tiktok_sessionid(sessionid)
    if not sid:
    ...
    if not re.match(r"^\d+", sid):
        raise InstagrapiSessionError(
            ...
            "Возможно, cookie битая или не из TikTok."
        )
    try:
    ...
    if not _client_looks_logged_in(cl):
        raise InstagrapiSessionError(
            "sessionid из браузера принят, но private API TikTok не отвечает "
            "(нужен для метрик). Войдите в аккаунт заново в антидетекте или "
            "проверьте tt_login/tt_password профиля."
        )
...
def ensure_instagrapi_client(
    ...
    *,
    ...
    if not pid:
        raise InstagrapiSessionError("Не выбран профиль для чекера TikTok.")
    ...
    if sessionid_provider is not None:
        try:
            ...
            if not sid:
                raise InstagrapiSessionError(
                    "В профиле нет cookie sessionid — войдите в TikTok в этом профиле."
                )
            cl = _new_client(fast=False, proxy=proxy_dsn)
            if path.is_file():
                try:
    ...
    raise InstagrapiSessionError(
        "Нет сохранённой сессии и нет логина/пароля TikTok в данных профиля. "
        "Укажите tt_login/tt_password или войдите в TikTok в браузере профиля."
    )
...
def invalidate_instagrapi_session(profile_id: str) -> None:
    """Удалить dump, если TikTok ответил login_required / session expired."""
    path = session_settings_path(profile_id)
    try:
