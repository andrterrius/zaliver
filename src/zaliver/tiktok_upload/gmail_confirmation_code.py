"""Чтение кода подтверждения TikTok из Gmail."""
...
_SUBJECT_CODE_RE = re.compile(
    r"(\d{6})\s+is\s+your\s+tiktok\s+code|"
    r"код\s+подтверждения.*?(\d{6})|"
    r"confirmation\s+code.*?(\d{6})|"
    r"tiktok\s+code.*?(\d{6})",
    re.IGNORECASE | re.DOTALL,
)
_IG_SENDER_RE = re.compile(r"tiktok|no-reply@mail\.tiktok\.com", re.IGNORECASE)
_IG_SUBJECT_HINT_RE = re.compile(
    r"tiktok\s+code|код.*tiktok|confirmation\s+code",
    re.IGNORECASE,
)
...
_META_SUBJECT_HINT_RE = re.compile(
    ...
    r"Use\s+\d{6,8}\s+to\s+log\s+into\s+TikTok|"
    r"log\s+back\s+into\s+TikTok|"
    r"to\s+log\s+into\s+TikTok",
    re.IGNORECASE,
)
_META_CODE_NEAR_RE = re.compile(
    r"Use\s+(\d{8})\s+to\s+log\s+into\s+TikTok|"
...
def _log(message: str) -> None:
    emit_tiktok_log(message, tag="[gmail]")
...
def _find_tiktok_row(page, *, exclude_codes=None):
    """Строка письма от TikTok с кодом (desktop tr.zA или mobile listitem)."""
    exclude = {
        str(c).strip()
    ...
    if _is_mobile_gmail_list(page) and _mobile_list_row_count(page) > 0:
        for row in _iter_mobile_list_rows(page):
            ...
            if not (
                ...
                or re.search(r"tiktok", subject, re.I)
            ):
                ...
                try:
                    ...
                    sender = ""
                if not re.search(r"tiktok", sender or "", re.I):
            ...
            try:
                if row.is_visible(timeout=300):
                    _log(f"Gmail (mobile): TikTok «{subject[:70]}»")
                    return row
            except Exception:
                continue
        return None
    ...
    candidates = [
        ...
        page.locator('span.zF[email="no-reply@mail.tiktok.com"]').locator(
            "xpath=ancestor::tr[contains(@class,'zA')][1]"
        ),
        page.locator('span.zF[name="TikTok"]').locator(
            "xpath=ancestor::tr[contains(@class,'zA')][1]"
        ),
        page.locator(_DESKTOP_ROW).filter(has_text=re.compile(r"tiktok", re.I)).filter(
            has_text=_CODE_RE
        ),
    ]
...
def _find_meta_auth_row(page, *, exclude_codes=None):
    """
    Строка письма Meta: «Use NNNNNNNN to log into TikTok» / Authenticate.
    ...
    if _is_mobile_gmail_list(page) and _mobile_list_row_count(page) > 0:
        # 1) «Use 65753582 to log into TikTok» — только .HhG5wd, сверху вниз.
        for row in _iter_mobile_list_rows(page):
            try:
                subject = _mobile_row_subject_text(row)
            except Exception:
...
def _extract_code_from_text(text: str) -> str | None:
    ...
    near = re.search(
        r"(?:confirmation\s+code|код\s+подтверждения|tiktok\s+code|"
    ...
    if near:
        return near.group(1)
    # Крупный одиночный код в письме TikTok (часто на отдельной строке).
    alone = re.findall(r"(?:^|\n)\s*(\d{6})\s*(?:$|\n)", raw)
    if len(alone) == 1:
        return alone[0]
    codes = _CODE_RE.findall(raw)
    if codes:
        # Первый 6-значный после упоминания TikTok / code.
        lower = raw.lower()
        for c in codes:
            idx = lower.find(c)
            window = lower[max(0, idx - 80) : idx + 20]
            if "tiktok" in window or "code" in window or "код" in window:
                return c
        return codes[0]
    return None
...
def _extract_meta_code_from_text(text: str) -> str | None:
    """8-значный код из письма Meta (Authenticate / log into TikTok)."""
    raw = (text or "").strip()
    if not raw:
        return None
...
def _click_tiktok_row(page, row) -> bool:
...
def _open_tiktok_row_and_read_code(page, row) -> str | None:
    """
    Вытащить 6-значный код из письма TikTok.
    Mobile: код из темы в списке, письмо не открываем.
    Desktop: открываем письмо; если за ``_OPEN_EMAIL_MAX_S`` не вышло — None.
    """
    # Mobile: «790186 is your TikTok code» уже в названии строки.
    if _is_mobile_gmail_list(page):
        ...
        if code:
            ...
            return code
        _log("Gmail (mobile): в названии нет кода TikTok — откроем письмо…")
    ...
    _log("Gmail: открываем письмо TikTok…")
    if not _click_tiktok_row(page, row):
        _log(
    ...
    while time.monotonic() < open_deadline:
        if _message_pane_open(page):
            ...
            continue
        try:
            again = _find_tiktok_row(page)
            if again is not None:
                _click_tiktok_row(page, again)
        except Exception:
            pass
        page.wait_for_timeout(400)
...
def _open_meta_row_and_read_code(page, row) -> str | None:
    """
    8-значный код Meta.
    Mobile: строго из темы «Use NNNNNNNN to log into TikTok», без открытия.
    """
    if _is_mobile_gmail_list(page):
        subject = _mobile_row_subject_text(row)
        m = _META_SUBJECT_USE_CODE_RE.search(subject or "")
    ...
    _log("Gmail: открываем письмо Meta (код TikTok / Authenticate)…")
    if not _click_tiktok_row(page, row):
        _log(
...
def fetch_tiktok_confirmation_code_from_gmail(
    gmail_page,
    *,
    ...
    while time.monotonic() < deadline:
        try:
            ...
            row = _find_tiktok_row(gmail_page, exclude_codes=exclude)
            if row is None:
                last_err = "письмо TikTok ещё не пришло"
                _log(f"Gmail: {last_err}, ждём…")
                gmail_page.wait_for_timeout(int(_EMAIL_POLL_S * 1000))
                continue

            code = _open_tiktok_row_and_read_code(gmail_page, row)
            if code:
                if code in exclude:
                    last_err = (
                    ...
                    gmail_page.wait_for_timeout(int(_EMAIL_POLL_S * 1000))
                    continue
                _log(f"Gmail: код подтверждения TikTok = {code}")
                return code
    ...
    raise RuntimeError(
        f"Gmail: не удалось получить код TikTok за {max_seconds:.0f} с"
