"""Редактирование профиля TikTok: bio/фото + смена юзернейма."""
...
EDIT_PROFILE_URL = "https://www.tiktok.com/accounts/edit/"
LANGUAGE_PREFERENCES_URL = "https://www.tiktok.com/language/preferences/"
ACCOUNTS_CENTER_URL = (
...
class TikTokEditProfileError(RuntimeError):
    """Ошибка редактирования профиля TikTok."""


def _log(message: str) -> None:
    emit_tiktok_log(message, tag="[tt-edit]")
...
def _upload_avatar(page, avatar_path: Path) -> None:
    ...
    if not path.is_file():
        raise TikTokEditProfileError(f"Файл аватарки не найден: {path}")
...
def _dismiss_photo_crop_if_present(page) -> None:
    """Если TikTok показал редактор фото — Save/Done/Apply."""
    patterns = (
...
def _username_candidate(base: str, *, suffix: int | None = None) -> str:
    ...
    if not base:
        raise TikTokEditProfileError("Пустой юзернейм после нормализации.")
    if suffix is None:
...
def _accounts_center_has_profiles_list(page) -> bool:
    """Вариант A: список Profiles с карточкой TikTok."""
    try:
        link = page.locator(
            'a[href*="/profiles/"][aria-label*="TikTok"]'
        ).first
...
def _click_own_tiktok_profile(page) -> None:
    """Клик по карточке TikTok в списке Profiles (оба варианта UI)."""
    link = page.locator('a[href*="/profiles/"][aria-label*="TikTok"]').first
    # Не кликать ссылки username/manage / photo/manage — только карточка профиля.
    n = 0
    try:
        n = page.locator('a[href*="/profiles/"][aria-label*="TikTok"]').count()
    except Exception:
        n = 0
    chosen = None
    for i in range(max(1, n)):
        ...
        if "/profiles/" in href and chosen is None:
            chosen = cand
    if chosen is None:
        # Иногда TikTok без слова в aria-label — берём /profiles/<id>.
        try:
    ...
    if chosen is None or chosen.count() <= 0:
        raise TikTokEditProfileError(
            "Не найдена карточка TikTok-профиля в Accounts Center."
        )
    chosen.wait_for(state="visible", timeout=30_000)
    _log("Accounts Center: открываем свой TikTok-профиль…")
    try:
...
def _open_own_tiktok_profile_from_accounts_center(page) -> None:
    """
    Открыть карточку TikTok для смены юзернейма.
    Вариант A (profiles): сразу клик по TikTok в списке.
    Вариант B (home): Главная → account_overview → TikTok.
    """
    ...
    _click_own_tiktok_profile(page)
...
def _fill_username_input(page, dialog, username: str) -> None:
    ...
    if not want:
        raise TikTokEditProfileError("Пустой юзернейм для ввода.")
...
def _verify_username_applied(page, expected: str, *, timeout_s: float = 20.0) -> None:
    ...
    while time.monotonic() < deadline:
        ...
        try:
            ...
            link = page.locator(
                f'a[aria-label*="{want}" i][aria-label*="TikTok" i]'
            ).first
            if link.count() > 0 and link.is_visible():
...
def _change_tiktok_language_to_russian(page) -> None:
    """
...
def _run_edit_profile_bio_avatar(
    ...
    *,
    ...
    accept_tiktok_cookie_consent_if_present(page, appear_seconds=4.0)
...
def run_tiktok_edit_profile(
    ...
    *,
    ...
    if avatar is not None and not avatar.is_file():
        raise TikTokEditProfileError(f"Файл аватарки не найден: {avatar}")
    ...
    _log("Edit profile: проверка сессии / главной TikTok…")
    verify_tiktok_home_available(
        page,
    ...
    if do_lang:
        _change_tiktok_language_to_russian(page)
    ...
    if want_username:
        applied = _change_tiktok_username(page, want_username)
