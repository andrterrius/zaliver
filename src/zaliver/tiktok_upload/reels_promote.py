"""Продвижение TikTok Reels: по ссылкам залитых роликов — подписка/лайк/коммент."""
...
from zaliver.tiktok_upload.reels_warmup import (
    _clamp_prob_pct,
...
_COMMENT_FIELD_SEL = (
    ...
    'textarea[aria-label*="Написать комментарий" i], '
    'input[aria-label*="Написать комментарий" i]'
)
_COMMENT_OPEN_SVG_SEL = (
    'svg[aria-label="Комментировать"], svg[aria-label="Comment"], '
    'svg[aria-label="Комментарии"], svg[aria-label="Comments"], '
    'svg[aria-label="Add a comment"], svg[aria-label="Написать комментарий"]'
)
_COMMENT_CLOSE_SVG_SEL = (
    'svg[aria-label="Закрыть"], svg[aria-label="Close"], '
    'svg[aria-label="Закрыть комментарии"]'
)
_COMMENT_POST_RE = re.compile(r"^(Опубликовать|Разместить|Post)$", re.I)
...
def _promotion_open_url(*, url: str, video_id: str) -> str:
    u = (url or "").strip()
    if u and "tiktok.com" in u.lower():
        return u
...
def _find_comment_post_button(page):
    ...
    candidates = [
        ...
        page.locator('form div[role="button"]:has-text("Разместить")'),
        page.locator('form div[role="button"]:has-text("Post")'),
        page.locator('div[role="button"]:has-text("Опубликовать")'),
        page.locator('div[role="button"]:has-text("Разместить")'),
...
@tiktok_entrypoint
def run_tiktok_profiles_promotion(
    page,
    *,
    ...
    **_ignored,
    ...
    _log("Продвижение: проверка сессии TikTok…")
    verify_tiktok_home_available(
        page,
    ...
    if not targets:
        raise RuntimeError(
            "Продвижение: нет целевых Тиктоков "
    ...
    _log(
        f"Продвижение: {len(targets)} Тиктоков по ссылкам"
    ...
    _log(
        f"Продвижение TikTok завершено "
